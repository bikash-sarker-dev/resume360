...
const Navbar = () => {
  ...
  return (
    ...
    <div className="relative z-50 w-full">
      ...
      >
        {/* Logo */}
        <div className="text-2xl font-serif ">Resume<span className="font-bold text-r-primary">360</span></div>

        {/* Desktop Navigation */}
        <ul className="hidden lg:flex space-x-6 text-lg font-medium">
          <Link className="cursor-pointer hover:text-gray-300 transition-all">Home</Link>
          <Link   className="cursor-pointer hover:text-gray-300 transition-all">Vlog</Link>
          <Link className="cursor-pointer hover:text-gray-300 transition-all">Info</Link>
        </ul>
        ...
        >
          <div className={`w-6 h-0.5 bg-r-accent mb-1 transition-all ${menuOpen ? "rotate-45 translate-y-1.5" : ""}`}></div>
          <div className={`w-6 h-0.5 bg-r-accent mb-1 transition-all ${menuOpen ? "opacity-0" : ""}`}></div>
          <div className={`w-6 h-0.5 bg-r-primary transition-all ${menuOpen ? "-rotate-45 -translate-y-1.5" : ""}`}></div>
        </button>
      </header>
      ...
      <div
        className={`lg:hidden fixed top-0 rounded-3xl backdrop-blur-3xl opacity-30  right-10 w-[300px]  text-r-text transition-all duration-300 ease-in-out ${
          menuOpen ? "opacity-100 z-0 translate-y-0 top-16" : "opacity-0  -translate-y-full pointer-events-none"
        }`}
      >
        <ul className="flex flex-col items-center space-y-4 py-6 text-lg">
          <Link className="cursor-pointer hover:text-gray-300" onClick={() => setMenuOpen(false)}>
            Home
          </Link>
          <Link className="cursor-pointer hover:text-gray-300" onClick={() => setMenuOpen(false)}>
            Vlog
